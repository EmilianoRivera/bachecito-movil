...
import {  addDoc, collection, doc, getDoc, query,where, getDocs, writeBatch} from 'firebase/firestore';
...
export default function Reporte({ route }) {
  ...
  const [confirmationModalVisible, setConfirmationModalVisible] = useState(false);
    ...
    const handleDescChange = (text) => {
      if (text.length <= 100) {
        if(/^[A-Za-z-., 0-9-áéíóú-]*$/.test(text)){
          setDescripcion(text);}
        } else {
        Alert.alert('¡Tranquilo! 😰', `Esa es una descripción muy larga, seguro que puedes escribir una menos detallada`);
        setDescripcion(text.substring(0, 100));
      }
    };
    //Agregar fotografía
...
useEffect(() => {
  ...
  const obtenerFolio = async () => {
    try {
      const folio = await obtenerFolioPorDireccion(direccion);
  ...
  obtenerFolio();
}, [direccion]); // Se ejecuta cada vez que la dirección cambia
...
var estadito = "Sin atender"
const enviarReporteABaseDeDatos = async () => {
  try {
    const imageURL = await subirImagenFirebaseStorage(imageUri);
    const folio = await obtenerFolioPorDireccion(direccion);

    // Verificar si la descripción está vacía y asignar "sin descripción" por defecto
    const descripcionReporte = descripcion.trim() ? descripcion : "Sin descripción";
    ...
    const reporte = {
      ...
      folio: folio,
      eliminado: false,
      contador: cantidadReportes + 1 // Incrementar el contador en 1
    };
  ...
  const handleReport = async () => {
    if (!isButtonDisabled) {
      ...
      alert("Por favor, llena todos los campos antes de reportar.");
    }
  };
                ...
                onPress={pickImage}>
                    ...
                    >
                        {imageUri ? (
                            ...
                            // Si no hay imagen, muestra el icono para agregar
